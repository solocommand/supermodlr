...
if (isset($field->values) && is_array($field->values) && count($field->values) > 0)
{
    ...
    $template = <<<EOT
        <select ng-model="model.{$field->path('_')}" ng-options="value for value in model.fields.{$field->path('_')}.values" {$angular_validation} autocomplete="off">
        </select>
EOT;
...
else
{
    ...
    $template = <<<EOT
        ...
        <input type="{$input_type}" ng-model="model.{$field->path('_')}" {$angular_validation} autocomplete="off" />
